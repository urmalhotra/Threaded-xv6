...
main(int argc, char *argv[])
{
   ...
   ppid = getpid();
  11:	e8 0f 04 00 00       	call   425 <getpid>
  16:	a3 30 0c 00 00       	mov    %eax,0xc30

   int fork_pid = fork();
  1b:	e8 7d 03 00 00       	call   39d <fork>
   if(fork_pid == 0) {
  ...
  2c:	50                   	push   %eax
  2d:	e8 13 04 00 00       	call   445 <join>
   assert(join_pid == -1);
  ...
  76:	e8 5a 03 00 00       	call   3d5 <kill>
  7b:	e8 25 03 00 00       	call   3a5 <exit>
   assert(fork_pid > 0);
  80:	6a 1e                	push   $0x1e
  ...
  9b:	eb b8                	jmp    55 <main+0x55>
     exit();
  9d:	e8 03 03 00 00       	call   3a5 <exit>

   assert(wait() > 0);
  a2:	e8 06 03 00 00       	call   3ad <wait>
  ...
  b4:	e8 57 04 00 00       	call   510 <printf>
   exit();
  b9:	e8 e7 02 00 00       	call   3a5 <exit>
   assert(wait() > 0);
  be:	6a 24                	push   $0x24
 ...
 209:	31 db                	xor    %ebx,%ebx
{
 ...
 215:	57                   	push   %edi
 216:	6a 00                	push   $0x0
 218:	e8 a0 01 00 00       	call   3bd <read>
    if(cc < 1)
...
int thread_join(void){
 337:	83 ec 20             	sub    $0x20,%esp
  wait_pid = join(&stack);
 33a:	50                   	push   %eax
 33b:	e8 05 01 00 00       	call   445 <join>
 340:	89 c3                	mov    %eax,%ebx
  free(stack);
 342:	58                   	pop    %eax
 343:	ff 75 f4             	push   -0xc(%ebp)
 346:	e8 65 03 00 00       	call   6b0 <free>
  return wait_pid;
}
 ...
 46c:	89 d1                	mov    %edx,%ecx
{
 ...
 4de:	56                   	push   %esi
 4df:	57                   	push   %edi
 4e0:	e8 e0 fe ff ff       	call   3c5 <write>
  while(--i >= 0)
 ...
 519:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 555:	57                   	push   %edi
 556:	56                   	push   %esi
 557:	e8 69 fe ff ff       	call   3c5 <write>
 ...
 5ae:	e8 12 fe ff ff       	call   3c5 <write>
        putc(fd, c);
 ...
 5bf:	57                   	push   %edi
 5c0:	56                   	push   %esi
 5c1:	e8 ff fd ff ff       	call   3c5 <write>
        putc(fd, c);
 ...
 66c:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 66f:	e8 51 fd ff ff       	call   3c5 <write>
        ap++;
...
malloc(uint nbytes)
{
 ...
 7a3:	e8 85 fc ff ff       	call   42d <sbrk>
  if(p == (char*)-1)
